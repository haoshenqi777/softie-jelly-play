...
Compiler: AssemblyScript 0.28.9, executed by Bun 1.4.2. Dependencies remain isolated from the application. From the repository root:
...
If registry downloads fail, `bun tools/wasm-compiler/bootstrap.mjs`
...
`bun tools/wasm-compiler/build-contact.mjs` builds
...
Run `bun test tests/contact-kernel.test.mjs`
...
`bun run build:wasm` builds both kernels. The volume generator currently extracts
references to `floorHeight` without declaring it, so volume compilation fails.
This predates the Bun migration and is intentionally left for a separate fix.
The checked-in WASM remains unchanged and is used by the application and parity
tests. A failed build can rewrite `kernel.ts`; review generated changes before committing.
